...
std::vector<sInstruction> Instruction;
...
void AsmInit()
{
	...
	RegisterByID.clear();
	for (int i=0;i<Register.size();i++){
		if (RegisterByID.size() <= Register[i].id)
	...
	if (p.type == ParamTInvalid){
		...
		if (p.deref){
			...
			if (p.disp == DispModeNone)
				strcpy(str, string2("[%s]", p.reg->name));
			else if (p.disp == DispMode8)
				strcpy(str, string2("[%s + %s]", p.reg->name, d2h(&p.value, 1)));
			else if (p.disp == DispMode16)
				strcpy(str, string2("[%s + %s]", p.reg->name, d2h(&p.value, 2)));
			else if (p.disp == DispMode32)
				strcpy(str, string2("[%s + %s]", p.reg->name, d2h(&p.value, 4)));
			else if (p.disp == DispModeSIB)
				strcpy(str, "SIB[...][...]");
			else if (p.disp == DispMode8SIB)
				strcpy(str, string2("[SIB... + %s]", d2h(&p.value, 1)));
			else if (p.disp == DispMode8Reg2)
				strcpy(str, string2("[%s + %s + %s]", p.reg->name, p.reg2->name, d2h(&p.value, 1)));
			else if (p.disp == DispModeReg2)
				strcpy(str, string2("[%s + %s]", p.reg->name, p.reg2->name));
		}else
			strcpy(str, p.reg->name);
	}else if (p.type == ParamTImmediate){
		//msg_write("im");
		if (p.deref)
			strcpy(str, string2("[%s]", d2h(&p.value, small_addr ? 2 : 4)));
		else
			strcpy(str, d2h(&p.value, absolute_size(p.size)));
	}else if (p.type == ParamTImmediateDouble){
		//msg_write("im");
		strcpy(str, string2("%s:%s", d2h(&((char*)&p.value)[4], 2), d2h(&p.value, small_param ? 2 : 4)));
	}
#if 0
	for (int i=0;i<Register.size();i++)
...
const char *Opcode2Asm(void *_code_,int length,bool allow_comments)
{
	...
	while(code < end){
		...
		// create asm code
			...
			// segment register?
			if (seg)
				strcat(str, string(seg->name, ": "));

			// command
			strcat(str, inst->name);

			// parameters
			if (small_param != mode16)
				strcat(str, small_param ? " word" : " dword");
			if (p1.type != ParamTNone){
				AddParam(param, p1);
				strcat(str, string(" ", param));
			}
			...
			if (allow_comments){
				...
				for (int ii=0;ii<48-l;ii++)
					strcat(str," ");
				strcat(str, "// ");
				strcat(str, d2h(code,long(cur) - long(code), false));
			}
			//msg_write(str);
			bufstr.append(str);
			bufstr.append("\n");
...
void GetParam(sInstParam &p, char *param, int pn)
{
	...
	// hex const
		...
		if (DebugAsm){
			if (p.type == ParamTImmediateDouble)
				printf("hex const:  %s:%s\n",d2h((char*)&p.value+4,2),d2h((char*)&p.value,4));
			else
				printf("hex const:  %s\n",d2h((char*)&p.value,4));
		}
	...
	// label substitude
		...
		if (CurrentAsmMetaInfo){
			...
			for (int i=0;i<CurrentAsmMetaInfo->GlobalVar.size();i++){
			...
			if (param[0]=='_'){
				...
				w.Add = 0;
				w.ParamNo = pn;
				CurrentAsmMetaInfo->WantedLabel.push_back(w);
				UnknownLabel = true;
				if (DebugAsm)
...
void OpcodeAddImmideate(char *oc, int &ocs, sInstParamFuzzy &ip, sInstParam &p, int pn)
{
	...
	if ((UnknownLabel) && ((p.type == ParamTImmediate) || (p.type == ParamTImmediateDouble)))
		for (int i=0;i<CurrentAsmMetaInfo->WantedLabel.size();i++){
	...
	// special stuff
		...
		if (strcmp(cmd,"bits_16")==0){
			...
			if (CurrentAsmMetaInfo){
				...
				b.Bits = 16;
				CurrentAsmMetaInfo->BitChange.push_back(b);
			}
			done=true;
		}
		...
		if (strcmp(cmd,"db")==0){
			...
			if (CurrentAsmMetaInfo){
				...
				d.Size = 1;
				CurrentAsmMetaInfo->Data.push_back(d);
			}
			buffer[AsmCodeLength++]=(char)(int)p1.value;
			done=true;
		}
		if (strcmp(cmd,"dw")==0){
			so("Daten:   2 byte");
			...
			*(short*)&buffer[AsmCodeLength]=(short)(int)p1.value;	AsmCodeLength+=2;
			done=true;
		}
		if (strcmp(cmd,"dd")==0){
			so("Daten:   4 byte");
			...
			*(int*)&buffer[AsmCodeLength]=(int)p1.value;	AsmCodeLength+=4;
			done=true;
		}
		...
		if (strcmp(cmd,"org")==0){
			if (CurrentAsmMetaInfo)
				CurrentAsmMetaInfo->CodeOrigin = (int)p1.value;
			done=true;
		}
		if (cmd[strlen(cmd)-1]==':'){
			so("Label");
			if (CurrentAsmMetaInfo){
				...
				CurrentAsmMetaInfo->Label.push_back(l);
				...
				for (int i=0;i<CurrentAsmMetaInfo->WantedLabel.size();i++)
					if (strcmp(CurrentAsmMetaInfo->WantedLabel[i].Name, cmd) == 0){
						sAsmWantedLabel *w = &CurrentAsmMetaInfo->WantedLabel[i];
						//msg_error("    -> wanted");
						//printf("cop %d   orig %d  add %d  pos %d  pil %d\n", (int)CurrentAsmMetaInfo->CurrentOpcodePos, (int)CurrentAsmMetaInfo->CodeOrigin, (int)w->Add, w->Pos, CurrentAsmMetaInfo->PreInsertionLength);
						...
						// remove from list
						CurrentAsmMetaInfo->WantedLabel.erase(CurrentAsmMetaInfo->WantedLabel.begin() + i);
						i--;
					}
			}
			done=true;
