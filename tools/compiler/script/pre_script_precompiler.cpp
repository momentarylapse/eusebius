...
void SetImmortal(CPreScript *ps)
{
	...
	for (int i=0;i<ps->Include.size();i++)
...
void CPreScript::AddIncludeData(CScript *s)
{
	...
	// classes
	for (int i=0;i<ps->Class.size() - PreClass.size();i++)
		Class.push_back(ps->Class[i + PreClass.size()]);
...
void CPreScript::HandleMacro(ps_line_t *l, int &line_no, int &NumIfDefs, bool *IfDefed, bool just_analyse)
{
	...
	/*	case MacroIfndef:
		...
		case MacroRule:
			...
			ln = -1;
			for (int i=0;i<ScriptLocation.size();i++)
				if (strcmp(ScriptLocation[i].Name, cur_name)==0)
					ln = ScriptLocation[i].Location;
			if (ln < 0){
				msg_error(string2("%s: unknown location in script rule: %s", Filename, cur_name));
