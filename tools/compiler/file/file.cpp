...
void add_created_dir(const char *dir)
{
...
void _file_set_error_(CFile *f,const char *str)
{
...
CFile *FileOpen(const char *filename)
{
	CFile *f = new CFile();
	if (f->Open(filename))
		return f;
...
CFile *FileCreate(const char *filename)
{
	CFile *f = new CFile();
...
CFile *FileAppend(const char *filename)
{
	CFile *f = new CFile();
...
bool CFile::Create(const char *filename)
{
	if (!SilentFileAccess){
...
bool CFile::Append(const char *filename)
{
	if (!SilentFileAccess){
...
void CFile::WriteComment(const char *str)
{
#ifdef FILE_COMMENTS_DEBUG
